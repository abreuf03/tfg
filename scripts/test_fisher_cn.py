...
from src.fisherkpp import (
    ...
    resolver_fisher_kpp_cn_semimplicito,
...
r = 1.0
K = 1.0
x0 = 20.0 #posición inicial del frente
u0 = dato_inicial_fisher_escalon(malla.x, K, x0)
U = resolver_fisher_kpp_cn_semimplicito(malla, D, r, K, u0)
...
plt.title("Perfiles de la solución de Fisher--KPP con Crank--Nicolson semimplícito")
plt.legend()
plt.grid(True)
...
for t_corte in [4.0, 6.0, 8.0]:
    ...
    v_num = coef[0]
    print(f"CN semimplícito, ajuste para t > {t_corte}: v = {v_num:.6f}")
...
plt.title("Posición del frente de Fisher--KPP con Crank--Nicolson semimplícito")
plt.legend()
plt.grid(True)

...
ARCHIVO_SOLUCION = RESULTADOS / "reducido_solucion_pulso_campo_medio.npz"
...
def main() -> None:
    if not ARCHIVO_SOLUCION.exists():
        raise FileNotFoundError(
            "No se encuentra la solución del pulso reducido:\n"
            f"{ARCHIVO_SOLUCION}\n"
            "Ejecuta primero simular_pulso_pde_reducido.py."
        )

    print("=== ANÁLISIS DEL PULSO DEL SISTEMA REDUCIDO ===")
    print(f"Cargando: {ARCHIVO_SOLUCION}")

    # --------------------------------------------------------------
    # Carga única de la solución y de sus metadatos
    # --------------------------------------------------------------
    with np.load(
        ARCHIVO_SOLUCION,
        allow_pickle=False,
        ...
        D = (
            float(np.asarray(datos["D"]).item())
            if "D" in datos.files
            else 1.0
        )
        rb = (
            float(np.asarray(datos["rb"]).item())
            if "rb" in datos.files
            else 0.1
        )
        re = (
            float(np.asarray(datos["re"]).item())
            if "re" in datos.files
            else 1.0
        )
        n0 = (
            float(np.asarray(datos["n0"]).item())
            if "n0" in datos.files
            else 1.0
        )
        dx = (
            float(np.asarray(datos["dx"]).item())
            if "dx" in datos.files
            else float(x[1] - x[0])
        )
        dt = (
            float(np.asarray(datos["dt"]).item())
            if "dt" in datos.files
            else float(t[1] - t[0])
        )
    ...
    # --------------------------------------------------------------
    forma_esperada = (t.size, x.size)

    if A.shape != forma_esperada:
        raise ValueError(
            f"A tiene forma {A.shape}; "
            f"se esperaba {forma_esperada}."
        )

    if I.shape != A.shape:
        raise ValueError(
            f"I tiene forma {I.shape}, "
            f"pero A tiene forma {A.shape}."
        )

    if posiciones.size != t.size:
        raise ValueError(
            "La trayectoria y el vector temporal "
            "tienen distinta longitud."
        )

    if not np.isfinite(A).all() or not np.isfinite(I).all():
        raise ValueError(
            "La solución contiene NaN o valores infinitos."
        )

    if np.min(A) < -1.0e-10:
        raise ValueError(
            "La densidad activa presenta valores negativos relevantes."
        )

    if np.min(I) < -1.0e-10:
        raise ValueError(
            "La densidad inactiva presenta valores negativos relevantes."
        )
    ...
    for t_min, t_max in ventanas:
        ...
        ajustes_velocidad[(t_min, t_max)] = ajuste
    ...
    tabla_velocidad = pd.DataFrame(filas_velocidad)

    tabla_velocidad.to_csv(
        RESULTADOS / "reducido_velocidad_pulso.csv",
        index=False,
    )
    ...
    ajuste_principal = ajustes_velocidad[ventana_principal]
    ...
    graficar_trayectoria_velocidad(
        ...
        velocidad_teorica=velocidad_teorica,
        ruta=FIGURAS / "reducido_campo_medio_velocidad_pulso.png",
    )

    # Se conserva como diagnóstico, aunque puede omitirse de la memoria.
    graficar_residuos(
        ajuste=ajuste_principal,
        ruta=FIGURAS / "reducido_campo_medio_residuos_velocidad.png",
    )
    ...
    # ==============================================================
    # 2. ASIMETRÍA DE PERFILES INDIVIDUALES
    # ==============================================================
    print("\n=== ASIMETRÍA DEL PULSO REDUCIDO ===")
    ...
    tabla_asimetria = pd.DataFrame(filas_asimetria)

    tabla_asimetria.to_csv(
        RESULTADOS / "reducido_asimetria_pulso.csv",
        index=False,
    )

    tabla_principal = tabla_asimetria[
        np.isclose(
            tabla_asimetria["fraccion_min"],
            banda_principal[0],
        )
        & np.isclose(
            tabla_asimetria["fraccion_max"],
            banda_principal[1],
        )
    ].copy()

    tabla_principal.to_csv(
        RESULTADOS / "reducido_asimetria_pulso_principal.csv",
        index=False,
    )
    ...
    graficar_ajuste_asimetria(
        ...
        ruta=FIGURAS / "reducido_campo_medio_asimetria_pulso.png",
    )
    ...
    # ==============================================================
    # 3. PERFIL COLAPSADO EN EL MARCO MÓVIL
    # ==============================================================
    print("\n=== PERFIL COLAPSADO DEL SISTEMA REDUCIDO ===")
    ...
    faltantes = [
        ...
        if clave not in ajuste_colapsado
    ]

    if faltantes:
        raise RuntimeError(
            "El ajuste de colas no devolvió todas las métricas: "
            + ", ".join(faltantes)
        )
    ...
    # Equivale a gamma_menos / gamma_mas.
    cociente = float(
    ...
    print(f"gamma_- = {gamma_menos:.8f}")
    print(f"gamma_+ = {gamma_mas:.8f}")
    print(f"longitud trasera = {longitud_menos:.8f}")
    print(f"longitud delantera = {longitud_mas:.8f}")
    print(f"gamma_- / gamma_+ = {cociente:.8f}")
    print(f"sqrt(2) - 1 = {cociente_teorico:.8f}")
    print(
        "Error porcentual = "
    ...
    tabla_colapsada.to_csv(
        RESULTADOS
        / "reducido_campo_medio_asimetria_pulso_colapsado.csv",
        index=False,
    )
    ...
    tabla_perfil.to_csv(
        RESULTADOS
        / "reducido_campo_medio_perfil_pulso_colapsado.csv",
        index=False,
    )
    ...
    graficar_pulso_colapsado(
        ...
        ruta=(
            FIGURAS
            / "reducido_campo_medio_asimetria_pulso_colapsado.png"
        ),
    )
